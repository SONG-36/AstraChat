...
function handler(event):
    try
        ...
        # 2. 加载会话上下文
        history = sessionManager.load(body.session_id)

        # 3. RAG 检索
        ragChunks = ragRetriever.search(body.message)

        # 4. 构造 prompt
        prompt = buildPrompt(history, ragChunks, body.message)

        # 5. 调用 LLM
        aiReply, usage = llmEngine.generate(prompt)

        # 6. 构造输出
        result = responseBuilder.build(aiReply, ragChunks, usage, body.session_id)

        # 7. 保存消息到 DynamoDB
        sessionManager.save(body.session_id, body.message, aiReply)

        return HTTP 200 + result

    except ValidationError as e:
        return HTTP 400 + error("INVALID_INPUT", e.message)

    except LLMError as e:
        return HTTP 500 + error("LLM_ERROR", e.message)

    except Exception as e:
        log(e)
        return HTTP 500 + error("SERVER_ERROR", "Internal error")
```
